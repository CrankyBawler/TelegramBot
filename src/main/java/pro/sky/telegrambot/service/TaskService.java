...
public class TaskService {
     ...
     */
    public Optional<NotificationTask> addTask(String command, Long chatId) {
        Matcher matcher = pattern.matcher(command);
        if (matcher.matches()) {
            String group1 = matcher.group(1);
            String group3 = matcher.group(3);
            LocalDateTime dateTime = LocalDateTime.parse(group1, DateTimeFormatter.ofPattern("dd.MM.yyyy HH:mm"));
            NotificationTask notificationTask = new NotificationTask(1L,chatId, group3, dateTime);
            return Optional.of(repository.save(notificationTask));
        }
    ...
    public List<NotificationTask> schedulledAnswer() {
        LocalDateTime now = LocalDateTime.now().truncatedTo(ChronoUnit.MINUTES);
        List<NotificationTask> actualTasks = repository.findByPerformDate(now);
